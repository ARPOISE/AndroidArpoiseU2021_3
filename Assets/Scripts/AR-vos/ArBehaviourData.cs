        ...
        protected override IEnumerator GetData()
        {
            ...
            while (string.IsNullOrWhiteSpace(ErrorMessage))
            {
                ...
                for (; ; )
                {
                    ...
                    while (!(request.result == UnityWebRequest.Result.ConnectionError || request.result == UnityWebRequest.Result.ProtocolError || request.result == UnityWebRequest.Result.DataProcessingError)
                    ...
                    if (request.result == UnityWebRequest.Result.ConnectionError)
                    {
                        if (setError)
                        {
                            ErrorMessage = "Layer '" + layerName + "': ConnectionError, " + request.error;
                            yield break;
                        }
                        continue;
                    }
                    if (request.result == UnityWebRequest.Result.ProtocolError)
                    {
                        if (setError)
                        {
                            ErrorMessage = "Layer '" + layerName + "': ProtocolError, " + request.error;
                            yield break;
                        }
                        continue;
                    }
                ...
                foreach (var url in assetBundleUrls)
                {
                    ...
                    if (request.result == UnityWebRequest.Result.ConnectionError)
                    {
                        if (setError)
                        {
                            ErrorMessage = "Icons '" + assetBundleUri + "': ConnectionError, " + request.error;
                            yield break;
                        }
                        continue;
                    }
                    if (request.result == UnityWebRequest.Result.ProtocolError)
                    {
                        if (setError)
                        {
                            ErrorMessage = "Icons '" + assetBundleUri + "': ProtocolError, " + request.error;
                            yield break;
                        }
                        continue;
                    }
                ...
                while ((innerLayer = innerLayers.Keys.FirstOrDefault()) != null)
                {
                    ...
                    for (; ; )
                    {
                        ...
                        if (request.result == UnityWebRequest.Result.ConnectionError)
                        {
                            if (setError)
                            {
                                ErrorMessage = "Layer '" + innerLayer + "': ConnectionError, " + request.error;
                                yield break;
                            }
                            continue;
                        }
                        if (request.result == UnityWebRequest.Result.ProtocolError)
                        {
                            if (setError)
                            {
                                ErrorMessage = "Layer '" + innerLayer + "': ProtocolError, " + request.error;
                                yield break;
                            }
                            continue;
                        }
                ...
                foreach (var tuple in webRequests)
                {
                    ...
                    if (request.result == UnityWebRequest.Result.ConnectionError)
                    {
                        if (setError)
                        {
                            ErrorMessage = "Bundle '" + assetBundleUri + "': ConnectionError, " + request.error;
                            yield break;
                        }
                        continue;
                    }
                    if (request.result == UnityWebRequest.Result.ProtocolError)
                    {
                        if (setError)
                        {
                            ErrorMessage = "Bundle '" + assetBundleUri + "': ProtocolError, " + request.error;
                            yield break;
                        }
                        continue;
                    }
                ...
                foreach (var tuple in webRequests)
                {
                    ...
                    if (request.result == UnityWebRequest.Result.ConnectionError)
                    {
                        if (setError)
                        {
                            ErrorMessage = "Image '" + triggerImageUri + "': ConnectionError, " + request.error;
                            yield break;
                        }
                        continue;
                    }
                    if (request.result == UnityWebRequest.Result.ProtocolError)
                    {
                        if (setError)
                        {
                            ErrorMessage = "Image '" + triggerImageUri + "': ProtocolError, " + request.error;
                            yield break;
                        }
                        continue;
                    }
